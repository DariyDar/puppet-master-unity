...
public class EnemyConfig : ScriptableObject
{
    ...
    public static EnemyConfig CreateArcherConfig()
    {
        ...
        config.damage = 10;     // GDD: 10 Damage
        config.attackSpeed = 0.5f;  // Slower attack - one shot every 2 seconds
        config.attackRange = 10f;    // Unity units (ranged) - max shooting distance
        ...
        config.detectionRange = 12f; // Unity units - sees far
        config.chaseRange = 15f;
        config.preferredRange = 7f;  // Unity units - wants to stay at this distance
        config.fearRange = 4f;       // Unity units - runs away if player closer than this
        config.xpReward = 40;
        config.meatMin = 10;
        config.meatMax = 18;
        config.woodMin = 8;
